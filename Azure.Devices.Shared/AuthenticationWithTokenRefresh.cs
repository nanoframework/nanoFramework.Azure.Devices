...
namespace Microsoft.Azure.Devices.Client
{
    ...
    public abstract class AuthenticationWithTokenRefresh : IAuthenticationMethod, IDisposable
    {
        ...
        /// </summary>
        public string GetToken(string iotHub)
        {
            if (!IsExpiring)
            {
                return _token;
            ...
            lock (_lock)
            {
                try
                {
                    ...
                    _token = SafeCreateNewToken(iotHub, _suggestedTimeToLiveSeconds);
        ...
        protected abstract string SafeCreateNewToken(string iotHub, int suggestedTimeToLive);
