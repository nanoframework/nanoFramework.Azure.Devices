...
namespace Microsoft.Azure.Devices.Client.TransientFaultHandling
{
    ...
    internal class Incremental : RetryStrategy
    {
        ...
        {
            if ((retryCount < 0) || (initialInterval.Ticks < 0) || (increment.Ticks < 0))
            {
        ...
        public override ShouldRetry GetShouldRetry()
        {
            return delegate (int currentRetryCount, Exception lastException, out TimeSpan retryInterval)
            {
                ...
                {
                    retryInterval = TimeSpan.FromMilliseconds((long)(_initialInterval.TotalMilliseconds + _increment.TotalMilliseconds * currentRetryCount));
                    return true;
                }
