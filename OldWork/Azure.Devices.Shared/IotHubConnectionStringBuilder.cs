...
namespace Microsoft.Azure.Devices.Client
{
    ...
    public sealed class IotHubConnectionStringBuilder
    {
        ...
        {
            Hashtable map = iotHubConnectionString.ToDictionary(ValuePairDelimiter, ValuePairSeparator);
        ...
        private void Validate()
        {
            ...
            if ((UsingX509Cert || Certificate != null) &&
                (!string.IsNullOrEmpty(SharedAccessKey)
                    || !string.IsNullOrEmpty(SharedAccessSignature)))
            {
        ...
        private static string GetConnectionStringValue(Hashtable map, string propertyName)
        {
        ...
        private static string GetConnectionStringOptionalValue(Hashtable map, string propertyName)
        {
            map.TryGetValue(propertyName, out string value);
            return value;
        }

        private static bool GetConnectionStringOptionalValueOrDefault(
            Hashtable map,
            string propertyName,
            TryParse tryParse,
            bool ignoreCase)
